...
Aplikace Seznam úkolů v **čistém PHP 8.3 (OOP)** — backend bez frameworku a bez Composeru. Frontend (šablona `templates/tasks.php`) je nastylovaný přes **Bootstrap 5** linkovaný z CDN — jediná externí závislost, žádný build ani npm. Slouží jako codebase pro výukový díl o Claude Code slash příkazech a Skills. Detailní popis architektury je v `ARCHITECTURE.md`.
